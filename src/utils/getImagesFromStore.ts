...
export const getGalleryImages = async (folder: string, id: string, currentUserId: string) => {
  const urls = await storage().ref().child(`${currentUserId}`).child(folder).child(`${id}`);
